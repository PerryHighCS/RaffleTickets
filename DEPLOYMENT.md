...
13. **Resonance timeout finalization**: When a timed active run expires, the next Resonance session access promotes every valid server-persisted draft from that run into a submitted response before clearing the active questions. No client-supplied late-submit flag is trusted. This is access-triggered rather than a background scheduler, so finalization may occur after the displayed deadline while retaining the server-recorded pre-deadline cutoff. Resonance still uses whole-session writes; horizontal scale-out safety depends on migrating all Resonance mutations to `SessionStore.updateAtomic(...)` under #313 before enabling multiple writers for one session.
