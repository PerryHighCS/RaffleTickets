...
- Validation rules: Student clients reject any older run/session snapshot, including when the active question changes or after an idle snapshot, but reset snapshot ordering state when either the session ID or student ID changes. Transitioning from idle/self-paced mode into a live run is a new run even when its question IDs are unchanged, so retained local answers for those questions remain available to pre-fill editable work while only the local submitted lock is cleared. A same-run snapshot may refresh an unchanged draft, but must not overwrite locally edited work. Student draft and submission payloads carry `activeQuestionRunStartedAt`; the server rejects a payload when that token does not match the current run. Revisited live questions may persist a revision draft even when that student already has a current-run response. At a timed deadline, the server finalizes all persisted drafts whose timestamps fall within the active run; stale, post-deadline, unknown-student, and inactive-question drafts are not submitted. Student answer and draft messages are bound to the connected socket's student ID; a payload cannot select another student record.
