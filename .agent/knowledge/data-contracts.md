...
- Validation rules: Student clients reject any older run/session snapshot, including when the active question changes or after an idle snapshot, but reset snapshot ordering state when either the session ID or student ID changes. Transitioning from idle/self-paced mode into a live run is a new run even when its question IDs are unchanged, so retained local answers for those questions remain available to pre-fill editable work while only the local submitted lock is cleared. A same-run snapshot may refresh an unchanged draft, but must not overwrite locally edited work. Student draft and submission payloads carry `activeQuestionRunStartedAt`; the server rejects a payload when that token does not match the current run. Revisited live questions may persist a revision draft even when that student already has a current-run response. Draft debounce is capped near a timed deadline, and a server-owned deadline task finalizes all persisted drafts whose timestamps fall within the active run; stale, post-deadline, unknown-student, and inactive-question drafts are not submitted. Deadline waits are segmented below Node's timer ceiling and retry after strict store failures. Student snapshots, submissions, and socket messages derive identity from a session-scoped participant capability cookie; client IDs cannot select another student record, and authenticated registration reloads reuse their current capability record.
